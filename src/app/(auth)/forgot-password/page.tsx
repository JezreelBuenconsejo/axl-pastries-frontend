    ...
    <div className="flex min-h-screen items-center justify-center w-full">
        ...
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleForgotPassword)} className="space-y-4">
              ...
              />
              <Button type="submit" className="w-full py-2" disabled={isLoading}>{isLoading ? "Loading..." : "Submit"}</Button>
            </form>
          </Form>
        </CardContent>
      </Card>
      ...
      <AlertDialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <AlertDialogContent className="bg-white">
          <AlertDialogHeader>
            <AlertDialogTitle className={cn("text-2xl", error ? "text-red-500" : "text-main-purple")}>{error ? "Error" : "Success"}</AlertDialogTitle>
